...
public class ClientController {
    ...
    @GetMapping("/list")
    public String daftarClient(ModelMap mm, @RequestParam(value = "key", required = false) String key,
            @PageableDefault(size = 10) Pageable page) {
        Page<Client> result;

        if (key != null) {

            result = clientDao.findByappNameContainingIgnoreCase(key, page);
        } else {

            result = clientDao.findAll(page);
        }
